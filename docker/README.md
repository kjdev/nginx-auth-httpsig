...
docker run --rm -p 8080:8080 \
  ...
  -e HTTPSIG_AGENT_ALLOW='agent.bot.example crawler.bot.example' \
  httpsig-proxy
```
...
| `HTTPSIG_AGENT_ALLOW` | — | space/comma-separated authorities; emits `auth_httpsig_agent_allow` per entry plus `auth_httpsig_key_cache_zone` and a `/httpsig_fetch` key-directory fetch location |
...
| `HTTPSIG_KEYID_FALLBACK_ALLOW` | — | set to emit `auth_httpsig_keyid_fallback_allow on`; only applies to `HTTPSIG_AGENT_ALLOW` keys, never `HTTPSIG_JWKS_FILE` |
...
At least one of `HTTPSIG_JWKS_FILE` or `HTTPSIG_AGENT_ALLOW` is
