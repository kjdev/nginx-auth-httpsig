...
=== TEST 4: absolute-form request-target derives @path/@query from the target URI, not the literal request-target
--- http_config
    auth_httpsig_jwks_file $TEST_NGINX_DATA_DIR/ed25519-jwks.json;
    auth_httpsig_profile   web-bot-auth;
--- config
...
# The wire request-target is the absolute-form
# "http://bot.example.test/t?a=1" (see the request line below), but
# @path/@query are defined against the target URI's path/query components
# (RFC 9421 SS2.2.6/SS2.2.7), which is "/t" / "a=1" regardless of
# request-target form. @authority still comes from the Host header
# ("localhost"), not from the absolute-form authority.
my $req = default_request(
    target  => '/t?a=1',
    headers => [['Signature-Agent', '"https://bot.example.test"']],
);
...
"Signature-Agent: \"https://bot.example.test\"\n"
    ...
    . "Signature: $sig\n"
--- request
GET http://bot.example.test/t?a=1
--- response_body chomp
verified=[1] keyid=[PdxXhn7dNHVGUgmgckoHmbcG9hsWAnqedH8vCuwIxMA] agent=[bot.example.test]
